...
function centerInput() {
  ...
  if (!wrapper) return;
  let currentW = 415, currentX = 35, currentY = 35 + systemLog.split("\n").length * 24 + 20;
  if (["INPUT", "GENERATIVE", "CONFIRM_EXIT"].includes(state)) currentY += 25;
  if (width < 600) { currentW = width * 0.8; currentX = width / 2 - min(200, width * 0.4); currentY = height - 120; }
...
function processText() {
  ...
  if (val.length > 0) {
    ...
    if (nounCount > 0) {
      ...
      else newLayer.extraThickness += nounCount * 1.5;
    }
...
function draw() {
  let wrapper = select("#terminal-input-wrapper");
  if (["MENU", "ABOUT", "TOUR", "SKIP_BOOT"].includes(state)) { if (wrapper) wrapper.hide(); background(2, 6, 2); } 
  else background(2, 6, 2, visualStack.some((l) => l.neonParamActive) ? 10 : 40);
...
function drawInterface() {
  ...
  if (width >= 600 && showPlaceholderLabel) { textSize(18); fill(0, 255, 65, 160); text("// " + currentPlaceholder, 35, 35 + systemLog.split("\n").length * 24 + 16); }
}
...
function renderLayer(layerData, globalNeonActive) {
  noFill();
  let baseWeight = layerData.isPlaceholder ? 0.8 : map(layerData.energy, 1, 50, 0.5, 2.5);
  let finalWeight = Math.pow(baseWeight, 2) * 0.7 + layerData.extraThickness;
  if (layerData.neonParamActive) finalWeight += 1.0;
  ...
  for (let i = 0; i < layerData.axes; i++) {
    ...
    for (let j = 0; j <= 1; j += step) {
      ...
      if (isP) { strokeWeight(finalWeight * 2); point(cx, cy); } 
      else {
